...
<html dir="ltr" mozdisallowselectionprint>
  ...
  <body tabindex="1">
    ...
    <button id="print_custom" class="floating-btn">
        🖨️ Imprimir
    </button>

    <button id="download_custom" class="floating-btn">
        💾 Guardar
    </button>
    </div> <!-- outerContainer -->
...
<script nonce="<?php echo \OC::$server->getContentSecurityPolicyNonceManager()->getNonce(); ?>">
	document.addEventListener('DOMContentLoaded', () => {
	const printBtn = document.getElementById('print_custom')
	const downloadBtn = document.getElementById('download_custom')
	...
	// 🟡 Evaluar estado inicial del botón original
	const isInitiallyDisabled = originalDownloadBtn?.hasAttribute('disabled')
	...
	const observer = new MutationObserver(() => {
		const isDisabled = originalDownloadBtn?.hasAttribute('disabled')
		if (isDisabled) {
			downloadBtn?.setAttribute('disabled', 'true')
			downloadBtn?.classList.add('disabled')
		} else {
	...
	if (originalDownloadBtn) {
		observer.observe(originalDownloadBtn, { attributes: true, attributeFilter: ['disabled'] })
	}
})
</script>
